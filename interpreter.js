...
(function () {
    ...
    function applyFilter(args){
        var env = E.getCloEnv(args[0]);
        // console.log(env);
        console.log(env[2]);
        if (A.isVarExp(A.getPrimAppExpArgs2(args[0]))){
            var variable1 = A.getVarExpId(A.getPrimAppExpArgs2(args[0]))
        } else {
            var variable1 = parseInt(A.getIntExpValue(A.getPrimAppExpArgs2(pred)));
        }
        console.log(variable1)
        // var bindings = E.lookup(env, variable1);
        // console.log(bindings);
        var primExp = E.getCloBody(fp.hd(args));
        var list = E.getListValue(args[1]);
        var primOp = A.getPrimAppExpPrim2(primExp);

        var f = function (x) {
            return true;
        }
        return E.createList(fp.filter(f, list));
    }
    ...
    function evalExp(exp, envir) {
        if (A.isIntExp(exp)) {
            ...
            var list = function (ints) {
                if (ints.length === 0) { //check isNull rather than === 0
                    return E.createList([]);
                } else {
                    var list = [];
                    ...
                    var result = list.map(Number);
                    return E.createList(result);
                }
            }
            return list(A.getListExpList(exp));
        }
            ...
            return applyPrimitive(A.getPrimAppExpPrim1(exp),
                SLang.absyn.getPrimAppExpArgs1(exp).map(function (arg) {
                    return evalExp(arg, envir);
                }));
        } else if (A.isPrimAppExp2(exp)) {
            return applyPrimitive(A.getPrimAppExpPrim2(exp),
                SLang.absyn.getPrimAppExpArgs2(exp).map(function (arg) {
                    return evalExp(arg, envir);
                }));
        } else {
